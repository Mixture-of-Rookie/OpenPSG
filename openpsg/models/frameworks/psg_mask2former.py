...
from mmdet.core import BitmapMasks, bbox, build_assigner
...
class PanopticSceneGraphMask2Former(Mask2Former):
    def __init__(
        self,
        backbone,
        neck=None,
        ...
        super(PanopticSceneGraphMask2Former, self).__init__(
            backbone,
            neck=neck,
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        if nopad_gt_masks is not None:
            gt_masks = nopad_gt_masks
        ...
        **kwargs
        ...
        if not is_testing:
            det_results = self.simple_test_sg_bboxes(imgs,
                                                     ...
                                                     **kwargs)
            pan_results = None
            det_masks = [r['masks'] for r in det_results]
            ...
            if assign_type == 'bbox':
                ...
                for i in range(len(img_metas)):
                    ...
                    gt_masks_i = gt_masks[i].masks[:, :img_height, :img_width]
                    nopad_gt_masks.append(gt_masks_i)
            ...
            det_masks = F.interpolate(
                torch.Tensor(det_masks[0]).unsqueeze(1),
        ...
        if 'dists' in det_results[0]:
            det_dists = [r['dists'] for r in det_results]
        else:
            det_dists = [
                F.one_hot(det_label,
        ...
        return det_bboxes, det_labels, target_labels, \
            det_dists, det_masks, pan_results, None, query_feats, nopad_gt_masks# }}}
    ...
    def simple_test_sg_bboxes(self, imgs, img_metas, rescale=False, **kwargs):# {{{
        ...
        for i in range(len(results)):
            ...
            results[i]['masks'] = results[i]['masks'].detach().cpu().numpy().astype('uint8')
            results[i]['labels'] = results[i]['labels'] + 1
    ...
    def relation_simple_test(# {{{
        ...
        bboxes, labels, target_labels, dists, pan_masks, pan_results, points, query_feats, _ \
            = self.detector_simple_test(
            img,
            img_meta,
            gt_bboxes,

...
class Encoder(nn.Module):
    ...
    def forward(self, x, num_objs):
        ...
        bs = len(num_objs)
        mask = torch.arange(0, max_num, device=x.device).long().unsqueeze(0).expand(
            bs, max_num).lt(num_atts.unsqueeze(1)).long().unsqueeze(-2)   # B * 1 * MAXN
...
class TransformerContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes):
        ...
        self.obj_layer = self.cfg.context_object_layer
        self.edge_layer = self.cfg.context_edge_layer
        assert self.obj_layer > 0 and self.edge_layer > 0
        ...
        self.lin_obj = nn.Linear(self.obj_dim + self.embed_dim + 128,
                                 self.hidden_dim)
        self.lin_edge = nn.Linear(self.obj_dim + self.embed_dim + self.hidden_dim,
                                  self.hidden_dim)
        ...
        self.context_obj = Encoder(
            EncoderLayer(
                self.hidden_dim, c(attn), c(ff), self.dropout_rate
            ),
            self.obj_layer,
        )
        self.context_edge = Encoder(
            EncoderLayer(
                self.hidden_dim, c(attn), c(ff), self.dropout_rate
            ),
            self.edge_layer

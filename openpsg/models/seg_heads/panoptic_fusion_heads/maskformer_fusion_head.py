...
class PSGMaskFormerFusionHead(BasePanopticFusionHead):
    ...
    def panoptic_postprocess(self, mask_cls, mask_pred, query_feat):
            ...
            for k in range(cur_classes.shape[0]):
                ...
                if mask_area > 0 and original_area > 0:
                    ...
                    if not isthing:
                        ...
                        query_feats_indicator[pred_class] = k
                    else:
                        panoptic_seg[mask] = (
                            pred_class + instance_id * INSTANCE_OFFSET)
                        query_feats_indicator[
                            pred_class + instance_id * INSTANCE_OFFSET] = k
                        instance_id += 1

        return panoptic_seg, cur_query_feats, query_feats_indicator
    ...
    def simple_test(self,
        ...
        for mask_cls_result, mask_pred_result, meta, query_feat in zip(
            ...
            result = dict()
            if panoptic_on:
                pan_results, query_feat, query_feats_indicator = self.panoptic_postprocess(
                    mask_cls_result, mask_pred_result, query_feat)
                result['pan_results'] = pan_results
                result['query_feats'] = query_feat
                result['query_feats_indicator'] = query_feats_indicator
